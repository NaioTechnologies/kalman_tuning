...
with open('file.csv', 'wb') as csvfile:
    ...
    spamwriter.writerow(['time'] + ['dt'] + ['meca_position_measure_received'] + ['meca_position_x'] + ['meca_position_y'] + ['visu_position_measure_received'] + ['visu_position_x'] + ['visu_position_y'] + ['visu_velocity_measure_received'] + ['visu_velocity_x'] + ['visu_velocity_y'] + ['imu_accel_measure_received'] + ['accel_mss_x'] + ['accel_mss_y'] + ['imu_theta_measure_received'] + ['imu_theta'] + ['kalman_position_x']+ ['kalman_position_y'])

    all_measures_read = False
    for line in f.readlines():

        if(all_measures_read):
            spamwriter.writerow([time] + [dt]+ [meca_position_measure_received] + [meca_position_x] + [meca_position_y] + [visu_position_measure_received] + [visu_position_x] + [visu_position_y] + [visu_velocity_measure_received] + [visu_velocity_x] + [visu_velocity_y] + [imu_accel_measure_received] + [accel_mss_x] + [accel_mss_y] + [imu_theta_measure_received] + [imu_theta] + [kalman_position_x] + [kalman_position_y])
            all_measures_read = False

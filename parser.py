...
with open('file.csv', 'wb') as csvfile:
    ...
    for line in f.readlines():
        ...
        if ("kalman_imu_accel" in line):
            imu_accel_measure_received = eval(line.split(" ")[-3])
            accel_mss_x = eval(line.split(" ")[-2])
            accel_mss_y = eval(line.split(" ")[-1])        # if ("reconstructed_heading" in line):
        ...
        if ("kalman theta" in line):
            kalman_theta = eval(line.split(" ")[-2])
